...
test_reference_aclesson = createResolveReferenceTest(
    tableName="aclesson",
    gqltype="AcLessonGQLModel",
    attributeNames=["id", "topic_id", "type_id", "lastchange"],
)
test_query_by_id = createByIdTest(
    tableName="aclesson", queryEndpoint="aclessonById"
)
test_query_page = createPageTest(
    tableName="aclesson", queryEndpoint="aclessonPage"
)
...
async def test_document_mutation():
    ...
    query = """
            mutation(
                ...
                operation: aclessonUpdate(aclesson: {
                id: $id,
